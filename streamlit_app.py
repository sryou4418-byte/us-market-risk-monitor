...
# v3.38.5 adaptive dashboard refinement — Streamlit engine + custom HTML/CSS skin.
st.markdown("""<style>
...
@media (orientation:portrait) and (min-width:781px){
  ...
  .r38-hero-main{grid-template-columns:minmax(0,1fr) minmax(190px,34%);min-height:118px}
  .r38-side-copy{max-width:230px}
...
st.markdown(f'''<div class="r38-head"><div><div class="r38-title">미국 증시 위험 모니터</div><div class="r38-subtitle">현재 시장 상황과 주요 위험 신호를 한눈에 확인하세요.</div><div class="r38-credit">Developed by 유유상 · v3.38.5</div></div><div class="r38-head-actions"><div class="r38-action">{now_kst.strftime('%Y.%m.%d')}　▣</div><div class="r38-action">↻　데이터 업데이트</div></div></div>''',unsafe_allow_html=True)
refresh_indicator()
...
st.markdown(f'<div class="r38-footer">Risk Monitor 3.38.5 · 화면 갱신 {datetime.now(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S KST")} · 캐시 즉시 표시 · 백그라운드 최신화</div>',unsafe_allow_html=True)
