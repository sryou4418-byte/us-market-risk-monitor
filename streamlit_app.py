...
# v3.39.2 UI state must be initialized before any theme/navigation rendering.
_qp = st.query_params
...
# v3.39.2 adaptive dashboard refinement — Streamlit engine + custom HTML/CSS skin.
st.markdown("""<style>
...
st.markdown(f'''<div class="r38-head"><div><div class="r38-title">미국 증시 위험 모니터</div><div class="r38-subtitle">현재 시장 상황과 주요 위험 신호를 한눈에 확인하세요.</div><div class="r38-credit">Developed by 유유상 · v3.39.2</div></div><div class="r38-head-actions"><div class="r38-action">{now_kst.strftime('%Y.%m.%d')}　▣</div><a class="r38-action" href="?view={_view}&theme={_theme_q}&refresh=1" target="_self">↻　데이터 업데이트</a></div></div>''',unsafe_allow_html=True)
...
if _view=="heatmap":
    ...
    _heatmap_html = f"""<!doctype html>
<html lang="ko">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<style>
  html,body{{margin:0;padding:0;width:100%;height:100%;overflow:hidden;background:{'#111827' if _theme=='dark' else '#ffffff'};}}
  .tradingview-widget-container{{width:100%;height:760px;}}
  .tradingview-widget-container__widget{{width:100%;height:calc(100% - 30px);}}
  ...
  .tradingview-widget-copyright a{{color:{'#8aa6ff' if _theme=='dark' else '#4b6fd8'};text-decoration:none;}}
</style>
</head>
<body>
...
<!-- TradingView Widget END -->
    ...
    st.iframe(_heatmap_html, width="stretch", height=780)
    st.stop()
...
st.markdown(f'<div class="r38-footer">Risk Monitor 3.39.2 · 화면 갱신 {datetime.now(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S KST")} · 캐시 즉시 표시 · 백그라운드 최신화</div>',unsafe_allow_html=True)
