...
# v3.39.1 UI state must be initialized before any theme/navigation rendering.
_qp = st.query_params
...
# v3.39.1 adaptive dashboard refinement — Streamlit engine + custom HTML/CSS skin.
st.markdown("""<style>
...
st.markdown(f'''<div class="r38-head"><div><div class="r38-title">미국 증시 위험 모니터</div><div class="r38-subtitle">현재 시장 상황과 주요 위험 신호를 한눈에 확인하세요.</div><div class="r38-credit">Developed by 유유상 · v3.39.1</div></div><div class="r38-head-actions"><div class="r38-action">{now_kst.strftime('%Y.%m.%d')}　▣</div><a class="r38-action" href="?view={_view}&theme={_theme_q}&refresh=1" target="_self">↻　데이터 업데이트</a></div></div>''',unsafe_allow_html=True)
...
if _view=="heatmap":
    ...
    <style>
      ...
      .hm-shell{{
        ...
        border-radius:12px;
        overflow:hidden;
        background:{'#111827' if _theme=='dark' else '#f4f6f9'};
      }}
      .hm-loader{{
        position:absolute;
        inset:0;
        ...
        color:{'#aeb8c5' if _theme=='dark' else '#7b8490'};
        background:linear-gradient(
          90deg,
          {'#151d28' if _theme=='dark' else '#f3f5f8'} 20%,
          {'#202a38' if _theme=='dark' else '#fafbfc'} 45%,
          {'#151d28' if _theme=='dark' else '#f3f5f8'} 70%
        );
      ...
      .hm-error{{
        ...
        text-align:center;
        font:600 13px -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
        color:{'#d8dee8' if _theme=='dark' else '#5f6875'};
        background:{'#151d28' if _theme=='dark' else '#f7f8fa'};
      }}
      ...
      .tradingview-widget-copyright a{{
        color:{'#8aa6ff' if _theme=='dark' else '#4b6fd8'};
    ...
    <div class="hm-shell">
      <div id="hm-loader" class="hm-loader">S&amp;P500 히트맵 불러오는 중…</div>
      <div id="hm-error" class="hm-error">
        TradingView 히트맵을 불러오지 못했습니다.<br>
        잠시 후 다시 시도해 주세요.
      </div>
    ...
    <script>
    (() => {{
      const loader = document.getElementById("hm-loader");
      const errorBox = document.getElementById("hm-error");
      const widgetHost = document.querySelector(".tradingview-widget-container");
      ...
      script.onload = () => {{
        // TradingView injects its iframe asynchronously after the script itself loads.
        let checks = 0;
    ...
    """
    components.html(_heatmap_html, height=780, scrolling=False)
    st.stop()
...
st.markdown(f'<div class="r38-footer">Risk Monitor 3.39.1 · 화면 갱신 {datetime.now(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S KST")} · 캐시 즉시 표시 · 백그라운드 최신화</div>',unsafe_allow_html=True)
